...
def main(
    ...
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Show details for all records, not just changes."),
) -> None:
    """
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        typer.secho(f"Loaded {len(records)} records. Replaying against {policy.name}...", fg=typer.colors.BLUE)
        ...
        for original_rec in records:
            ...
            try:
                ...
                new_rec = decide(original_req, config=config, store=store, loaded_policy=loaded_policy)
            except Exception as e:
                errors += 1
                continue
        ...
        # 4. Report
        typer.echo("")
        typer.secho(f"--- Diff Report ---", bold=True)
        ...
        if not changes:
            typer.secho("No regressions found. verdicts match 100%.", fg=typer.colors.GREEN, bold=True)
            return
