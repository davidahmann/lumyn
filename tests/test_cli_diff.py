    ...
    records = [
        {
            ...
            "request": {
                ...
                "action": {"type": "support.refund", "intent": "test", "amount": {"value": 10, "currency": "USD"}},
                "evidence": {"risk": "low"},
        ...
        },
        {
            ...
            "request": {
                ...
                "action": {"type": "support.refund", "intent": "test", "amount": {"value": 1000, "currency": "USD"}},
                "evidence": {"risk": "high"},
                "context": {"mode": "digest_only", "digest": "sha256:1111111111111111111111111111111111111111111111111111111111111111"}
            }
        }
    ]
...
def test_diff_regressions(diff_workspace):
    ...
    assert result.exit_code == 1, f"Expected 1 (Regressions), got {result.exit_code}. Stdout: {result.stdout} Stderr: {result.stderr}"
...
def test_diff_clean(diff_workspace):
    ...
    assert result.exit_code == 0, f"Expected 0 (Clean), got {result.exit_code}. Stdout: {result.stdout} Stderr: {result.stderr}"
